<div class="inline">
    ...
    <x-jet-confirmation-modal wire:model="modal">
        <x-slot name="title">
            {{ __('incidents.delete_incident.modal.title') }}
        </x-slot>

        <x-slot name="content">
            {{ __('incidents.delete_incident.modal.text_r1', ['title' => $incident->title]) }}<br>
            {{ __('incidents.delete_incident.modal.text_r2') }}
        </x-slot>
        ...
        <x-slot name="footer">
            ...
            <x-jet-danger-button class="ml-2" wire:click="save" wire:loading.attr="disabled">
                {{ __('incidents.delete_incident.modal.delete_button') }}
            </x-jet-danger-button>
        </x-slot>
