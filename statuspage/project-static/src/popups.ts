...
export function initPopups(){
    for (const component of getElements<HTMLDivElement>('div[data-last-update]')) {
        const timestamp = component.getAttribute('data-last-update') ?? 'Unknown';

        const tooltip = document.createElement('div');
        tooltip.classList.add('hidden', 'bg-zinc-400', 'dark:bg-zinc-900', 'px-2', 'py-1', 'rounded-md');
        tooltip.innerHTML = `Last Update: ${timestamp}`;
