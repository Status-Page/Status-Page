...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        })->everyFiveMinutes()->description('Caches Metric Data (24 Hours)')->runInBackground();
        $schedule->call(function (){
            if(Setting::getBoolean('metrics_cache')){
                $metrics = Metric::all();
                foreach ($metrics as $metric){
        ...
        })->everyFiveMinutes()->description('Caches Metric Data (48 Hours)')->runInBackground();
        $schedule->call(function (){
            if(Setting::getBoolean('metrics_cache')){
                $metrics = Metric::all();
                foreach ($metrics as $metric){
        ...
        })->everyFiveMinutes()->description('Caches Metric Data (72 Hours)')->runInBackground();
        $schedule->call(function (){
            if(Setting::getBoolean('metrics_cache')){
                $metrics = Metric::all();
                foreach ($metrics as $metric){
        ...
        })->everyFiveMinutes()->description('Caches Metric Data (168 Hours)')->runInBackground();
    }
